...
fn finish_contract(mut paths: serde_json::Map<String, Value>) -> Value {
    ...
    paths.insert("/v1/claimable-bogs".into(),json!({"post":{"operationId":"create_claimable_bog","description":"Create a one-hour temporary Bog without sign-in when enabled. Supply a private random 32-byte recovery secret as 64 hex characters and a stable Idempotency-Key. Retry with both values to rotate and recover the temporary credential. Never log or display the returned credential.","security":[],"parameters":[{"name":"Idempotency-Key","in":"header","required":true,"schema":{"type":"string","maxLength":128}}],"requestBody":{"required":true,"content":{"application/json":{"schema":{"type":"object","additionalProperties":false,"properties":{"name":{"type":"string"},"definition":{"type":"object"},"recovery_secret":{"type":"string","pattern":"^[0-9a-fA-F]{64}$"}},"required":["name","recovery_secret"]}}}},"responses":{"201":{"description":"Temporary Bog with private one-time bearer credential, routes, and Unix expiry"},"400":response(reference("Error")),"409":response(reference("Error")),"429":response(reference("Error"))}}}));
    paths.insert("/v1/claimable-bogs/{bog_id}/claim".into(),json!({"post":{"operationId":"create_claim_link","description":"One-Bog temporary bearer requests a new single-use claim URL; it expires in at most 15 minutes and never beyond the Bog's one-hour lifetime.","security":[{"bearer":[]}],"parameters":[{"name":"bog_id","in":"path","required":true,"schema":{"type":"string","format":"uuid"}}],"responses":{"200":{"description":"Claim URL and Unix expiry; no data or credential"},"401":response(reference("Error")),"404":response(reference("Error"))}}}));
    paths.insert("/claim/{code}/status".into(),json!({"get":{"operationId":"claimable_status","description":"Public status-only lookup for a claim link: active, claimed, or expired. Never returns data or credentials.","security":[],"parameters":[{"name":"code","in":"path","required":true,"schema":{"type":"string"}}],"responses":{"200":{"description":"Claim state and link expiry"}}}}));
    paths.insert("/claim/{code}".into(),json!({"get":{"operationId":"view_claim","description":"Human claim page. GitHub sign-in is required to accept.","security":[],"parameters":[{"name":"code","in":"path","required":true,"schema":{"type":"string"}}],"responses":{"200":{"description":"HTML claim page"}}},"post":{"operationId":"accept_claim","description":"Signed-in human transfers the same Bog into an accessible workspace; requires exact Origin and x-csrf-token from /console-session. Revokes all temporary credentials.","security":[{"browserSession":[]}],"parameters":[{"name":"code","in":"path","required":true,"schema":{"type":"string"}}],"requestBody":{"required":true,"content":{"application/json":{"schema":{"type":"object","properties":{"workspace_id":{"type":"string","format":"uuid"}}}}}},"responses":{"200":{"description":"Claimed Bog and destination workspace"},"401":response(reference("Error")),"403":response(reference("Error")),"409":response(reference("Error")),"429":response(reference("Error"))}}}));
