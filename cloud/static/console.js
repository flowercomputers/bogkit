...
async function refresh() {
  ...
  for (const bog of bogs) {
    ...
    info.append(element('small', bog.id)); if (bog.sandbox) info.append(element('small', 'Temporary · expires ' + new Date(bog.sandbox.expires_at * 1000).toLocaleString())); const requests = element('td', '—'), waiting = element('td', '—'); row.append(info, element('td', bog.status || 'saved'), storage, requests, waiting); row.append(controls);
