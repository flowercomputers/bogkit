...
- `fen` — the chat example grown up: a mobile-friendly chat where every message is incrementally indexed for hybrid (BM25 + HNSW) search, deletes retract from every view atomically, and history persists across restarts. `cargo run -p fen`, then open http://localhost:3001
