...
vars. `Grep` disappears entirely on retraction, because it existed only in the
retracted session, and every other figure is back to exactly what it was —
...
Everything beyond `demo` runs against your own Claude Code history in
`~/.claude/projects`:
...
```
replayed 2,926 calls spanning 269h · window = last 24h
1,391 calls still inside the window
```
...
risking the primary ones. That also means `window` is a pure function of
(corpus, hours) — it replays, prints, and deletes its database, so there is no
second piece of state to keep in sync with the main views.
...
Worth separating, because two different subjects live in this repo.
...
they carry. Replaying a historical corpus through it naively stamps a year of
transcripts as all arriving "now", and every window returns everything.
`Retain::with_clock` is the way out: bog-bench drives a synthetic clock from the
transcripts' own timestamps and commits in event order, one transaction per hour
of corpus time, so expiry follows event time. That is what makes the `window`
command above work at all.

The deeper version of that observation, for whoever builds on this: an event-time
window ultimately wants a *ranked* view scored by the event timestamp, so the
query becomes a read-time range scan rather than a replay through a fake clock.
The clock trick is the right hackathon-scale answer; a `Ranked` branch on the
main pipeline is probably the right engine-scale one.
