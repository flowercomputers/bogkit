...
Implemented `scripts/cloud/composable_capacity.py` with Python standard library only. Syntax compilation and `--help` succeeded. The integration owner subsequently ran the release image successfully; see [measured results](bog-composable-capacity-results.md). No live API or production state was exercised.

The script drives actual HTTP endpoints on a loopback-only service. It requires a container labeled `bog.capacity=local`, the literal synthetic owner token `capacity-local-fixture-not-a-production-secret`, the exact dummy native-auth configuration below, composable enabled, eight resident slots, and an exact bind mount of a fresh temporary root to `BOG_CLOUD_ROOT`. It checks cgroup v2 limits of 1 GiB and one CPU. It rejects any existing Bogs/accounts in the initialized registry. The harness stops the labeled local container before inserting three ordinary synthetic accounts and personal workspaces in a short host SQLite transaction, then starts it again and waits for readiness; each provisions three Bogs through HTTP. The ninth Bog uses records-v1 to exercise compatibility and resident admission together. This permits nine retained Bogs without overriding either the normal three-Bog account quota or the eight resident-worker limit. These credentials are fixtures, never external account credentials, and are omitted from reports.
...
Bind mount ownership may be changed by the image entrypoint to UID 10001. The host user must have permission to write the fresh SQLite fixture; address permissions only within the dedicated temporary root. The script never creates/removes containers or deletes data. It stops and starts the dedicated container before the workload to seed SQLite safely without concurrent host/Linux connections. The optional `--restart` performs a separate measured persistence restart after the workload. Container setup and cleanup remain the integration owner's responsibility. Use a fresh root/container for each run.
