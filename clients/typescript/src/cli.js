...
import {Client,connect,installPrivate} from './index.js';
...
try {
  ...
  if(a.help){console.log('bog-cloud [--config PRIVATE_FILE | --auth-file PRIVATE_FILE] [--bog-id ID] COMMAND\nCommands: connect, resources, routes, diagnostics, get KEY, put KEY --input JSON, remove KEY, batch --input JSON, batch-get KEY..., list, search, wait, request-status ID, create, add-search, install, query, top, explain, cleanup-preview, cleanup-execute, delete-sandbox');}
  else {
